...
export const routingRouter = new Hono<Env>();

routingRouter.post("/tickets/routing-rules", tenantAuth, async (c) => {
...
routingRouter.get("/tickets/routing-rules", tenantAuth, async (c) => {
...
routingRouter.post("/tickets/:id/route", tenantAuth, async (c) => {
  const tenant = c.get("tenant");
...
routingRouter.put("/tickets/:id/assign", tenantAuth, async (c) => {
  const tenant = c.get("tenant");
  const id = c.req.param("id");
  const ticket = await dbStore.tickets.findOne(id);
  if (!ticket) {
