...
**Phase:** 2 · **Priority:** Medium · **Status:** `[ ] Todo` · **Depends on:** 011, 017
...
- [ ] New route `GET /api/dashboard/analytics` registered in `apps/api/src/routes/dashboard.ts`.
- [ ] Returns `{ leadCount, conversionRate, avgVelocityDays, slaBreachCount, byOwner: [...] }` per tenant.
- [ ] RLS-enforced via `withTenant`; cross-tenant requests return 0 or 403.
- [ ] OpenAPI doc auto-generated.
- [ ] Tests in `packages/testing/src/dashboard-analytics.test.ts` cover happy path + RLS isolation + empty tenant.
- [ ] Legacy `tickets/TICKET004.md` updated to `Status: completed`.
