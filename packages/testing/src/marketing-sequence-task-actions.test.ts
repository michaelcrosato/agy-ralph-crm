...
describe("Marketing Sequence Task Actions Tests (Task 0220)", () => {
  ...
  describe("API Endpoints & Validation Rules", () => {
    it("should allow creating a task step via API", async () => {
      let sequenceId = "";
      await withTenant(orgA, mockDb, async () => {
        const seq = await dbStore.marketingSequences.insert({
          orgId: orgA,
          name: "Task Follow-up Sequence",
          status: "active",
          senderType: "system",
        });
        sequenceId = seq.id;
      ...
      const res = await app.request(`/api/sequences/${sequenceId}/steps`, {
        ...
        body: JSON.stringify({
          ...
          delayDays: 1,
          stepType: "task",
          taskSubject: "Call new lead",
          taskBody: "Lead registered. Please call them ASAP.",
          taskDueDays: 2,
        }),
      });
      ...
      expect(body.step.stepType).toBe("task");
      expect(body.step.taskSubject).toBe("Call new lead");
      expect(body.step.taskBody).toBe(
        "Lead registered. Please call them ASAP.",
      );
      expect(body.step.taskDueDays).toBe(2);
      expect(body.step.templateId).toBeNull();
    });

    it("should enforce validation rules for task step creation", async () => {
      let sequenceId = "";
      await withTenant(orgA, mockDb, async () => {
        const seq = await dbStore.marketingSequences.insert({
          orgId: orgA,
      ...
      // 1. Task step missing taskSubject -> 400
      const missingSubjectRes = await app.request(
    ...
    it("should block Tenant B from adding task steps to Tenant A's sequence", async () => {
      let sequenceId = "";
      await withTenant(orgA, mockDb, async () => {
        const seq = await dbStore.marketingSequences.insert({
          orgId: orgA,
      ...
      const res = await app.request(`/api/sequences/${sequenceId}/steps`, {
        ...
        body: JSON.stringify({
          stepNumber: 1,
          stepType: "task",
          taskSubject: "Malicious attempt",
        }),
      });

      // Assert that Tenant B is barred due to RLS/NotFound sequence
      expect(res.status).toBe(404);
    });
  });

  describe("Task Action Execution Engine & Personalization", () => {
    it("should generate a personalized CRM Task activity and link it to the Lead cleanly", async () => {
      let sequenceId = "";
      let leadId = "";
      ...
      await withTenant(orgA, mockDb, async () => {
        ...
        const seq = await dbStore.marketingSequences.insert({
          orgId: orgA,
          name: "Personalized Task Sequence",
          status: "active",
          senderType: "system",
        });
        sequenceId = seq.id;

        // 3. Create a Task Step with dynamic recipient personalization tags
        await dbStore.marketingSequenceSteps.insert({
          ...
          delayDays: 0,
          stepType: "task",
          taskSubject: "Call {{lead.email}} at Acme",
          taskBody: "Lead company name is {{lead.company}}.",
          taskDueDays: 3,
        });
      ...
      await withTenant(orgA, mockDb, async () => {
        ...
        // 6. Assert that CRM task was created correctly
        const activities = await dbStore.activities.findMany();
        expect(activities.length).toBe(1);
        const act = activities[0];
        expect(act.type).toBe("task");
        expect(act.subject).toBe("Call sales-lead@tenant-a.com at Acme");
        expect(act.body).toBe("Lead company name is Acme Sales Corp.");
        expect(act.creatorId).toBe("00000000-0000-0000-0000-000000000000");
        expect(act.orgId).toBe(orgA);
        expect(act.dueDate).toBeDefined();
    ...
    it("should assert strict tenant RLS isolation on executed task steps and CRM activities", async () => {
      let seqA = "";
      let seqB = "";
      let leadA = "";
      let leadB = "";
      ...
      await withTenant(orgA, mockDb, async () => {
        ...
        const activities = await dbStore.activities.findMany();
        expect(activities.length).toBe(1);
        expect(activities[0].subject).toBe("Tenant A Task");
      ...
      await withTenant(orgB, mockDb, async () => {
        ...
        const activities = await dbStore.activities.findMany();
        expect(activities.length).toBe(1);
        expect(activities[0].subject).toBe("Tenant B Task");
