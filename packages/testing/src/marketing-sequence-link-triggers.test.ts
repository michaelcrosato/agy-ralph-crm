...
describe("Marketing Sequence Link Actions Engine Tests (Task 0197)", () => {
  ...
  it("should configure, trigger, and execute link actions with strict tenant RLS isolation", async () => {
    let seqId = "";
    let stepId = "";
    let leadId = "";
    let templateId = "";
    ...
    await withTenant(orgA, mockDb, async () => {
      const seq = await dbStore.marketingSequences.insert({
        orgId: orgA,
        name: "Demo Journey",
        description: "Testing sequence link click triggers",
        status: "active",
      });
      seqId = seq.id;
      ...
      const template = await dbStore.emailTemplates.insert({
        orgId: orgA,
        name: "Day 1 Welcome",
        subject: "Click this link: {{lead.company}}",
        body: "Hey, check out this link: https://example.com/demo",
      });
    ...
    // 2. Configure link click actions via API under Tenant A
    const postRes1 = await app.request(
      `/api/sequences/steps/${stepId}/link-actions`,
      {
        method: "POST",
        headers: {
    ...
    const postRes2 = await app.request(
      `/api/sequences/steps/${stepId}/link-actions`,
      {
        ...
        body: JSON.stringify({
          ...
          actionConfig: {
            subject: "Prospect clicked wildcard link",
            body: "Follow up quickly!",
            dueDateOffsetDays: 3,
          },
        }),
      },
    ...
    // 3. Verify link actions are configured correctly via API
    ...
    await withTenant(orgA, mockDb, async () => {
      ...
      // 5. Simulate link click event via Hono REST API click redirect endpoint
      const clickRes = await app.request(
        `/api/public/emails/track/click/${tracker.token}?target=https://example.com/demo`,
        {
          method: "GET",
        },
      );
      expect(clickRes.status).toBe(302);
      expect(clickRes.headers.get("location")).toBe("https://example.com/demo");

      // 6. Assert click actions were triggered and executed successfully
      ...
      // One email activity from step execution, and one task activity from link action trigger
      expect(activities.length).toBe(2);
      const task = activities.find((act) => act.type === "task");
      expect(task).toBeDefined();
      expect(task?.subject).toBe("Prospect clicked wildcard link");
      expect(task?.body).toBe("Follow up quickly!");
      ...
      const clickTriggerAudit = auditLogs.find(
        (log) => log.action === "link_trigger_executed",
      );
      expect(clickTriggerAudit).toBeDefined();
      expect(clickTriggerAudit?.recordType).toBe(
        "marketing_sequence_memberships",
      );
    });

    // 7. Strict Tenant RLS Isolation check
    // Tenant B attempts to list Tenant A's link actions -> expect 0 items due to RLS findMany filter
    ...
    // Tenant B attempts to delete Tenant A's link actions -> expect 404
    const actionId = getData.data[0].id;
    const badDeleteRes = await app.request(
      `/api/sequences/steps/link-actions/${actionId}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${tokenTenantB}`,
