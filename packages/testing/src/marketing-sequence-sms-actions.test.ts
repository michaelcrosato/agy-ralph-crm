...
describe("Marketing Sequence SMS Actions Tests (Task 0221)", () => {
  ...
  describe("API Endpoints & Validation Rules", () => {
    it("should allow creating an SMS step via API", async () => {
      let sequenceId = "";
      ...
      const res = await app.request(`/api/sequences/${sequenceId}/steps`, {
        ...
        body: JSON.stringify({
          ...
          delayDays: 1,
          stepType: "sms",
          smsMessage: "Hello {{lead.email}}, welcome to tenant A!",
        }),
      });
      ...
      expect(body.step.stepType).toBe("sms");
      expect(body.step.smsMessage).toBe(
        "Hello {{lead.email}}, welcome to tenant A!",
      );
    });

    it("should enforce validation rules for SMS step creation", async () => {
      let sequenceId = "";
      await withTenant(orgA, mockDb, async () => {
        const seq = await dbStore.marketingSequences.insert({
          orgId: orgA,
      ...
      // SMS step missing smsMessage -> 400
    ...
    it("should block Tenant B from adding SMS steps to Tenant A's sequence", async () => {
      let sequenceId = "";
      await withTenant(orgA, mockDb, async () => {
        const seq = await dbStore.marketingSequences.insert({
          orgId: orgA,
  ...
  describe("SMS Action Execution Engine & Personalization", () => {
    it("should generate a personalized CRM SMS activity and link it to the Lead cleanly", async () => {
      let sequenceId = "";
      let leadId = "";
      ...
      await withTenant(orgA, mockDb, async () => {
        ...
        const seq = await dbStore.marketingSequences.insert({
          orgId: orgA,
          name: "Personalized SMS Sequence",
          status: "active",
          senderType: "system",
        });
        sequenceId = seq.id;

        // 3. Create an SMS Step with dynamic recipient personalization tags
        await dbStore.marketingSequenceSteps.insert({
          ...
          delayDays: 0,
          stepType: "sms",
          smsMessage: "Hello {{lead.email}} at {{lead.company}}!",
        });
      ...
      await withTenant(orgA, mockDb, async () => {
        ...
        // 6. Assert that CRM activity of type 'sms' was created correctly
        const activities = await dbStore.activities.findMany();
        expect(activities.length).toBe(1);
        const act = activities[0];
        expect(act.type).toBe("sms");
        expect(act.subject).toBe("Outbound SMS");
    ...
    it("should assert strict tenant RLS isolation on executed SMS steps and CRM activities", async () => {
      let seqA = "";
      let seqB = "";
      let leadA = "";
      let leadB = "";
      ...
      await withTenant(orgA, mockDb, async () => {
        ...
        await dbStore.marketingSequenceSteps.insert({
          ...
          delayDays: 0,
          stepType: "sms",
          smsMessage: "Tenant A SMS message",
        });
      ...
      await withTenant(orgB, mockDb, async () => {
        ...
        await dbStore.marketingSequenceSteps.insert({
          ...
          delayDays: 0,
          stepType: "sms",
          smsMessage: "Tenant B SMS message",
        });
      ...
      await withTenant(orgA, mockDb, async () => {
        ...
        const activities = await dbStore.activities.findMany();
        expect(activities.length).toBe(1);
        expect(activities[0].body).toBe("Tenant A SMS message");
      ...
      await withTenant(orgB, mockDb, async () => {
        ...
        const activities = await dbStore.activities.findMany();
        expect(activities.length).toBe(1);
        expect(activities[0].body).toBe("Tenant B SMS message");
